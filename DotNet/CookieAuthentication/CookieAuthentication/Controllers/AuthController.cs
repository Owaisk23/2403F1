...
namespace SessionAuthentication.Controllers
{
    ...
    {
        private readonly AuthuserContext db;
        public AuthController(AuthuserContext _db)
        {
            db = _db;
        }
        ...
        public IActionResult Login(User user)
        {
            ...
            if (checkUser != null)
            {
                ...
                if (verifyPass == PasswordVerificationResult.Success && checkUser.RoleId == 2)
                {
                ...
                else if (verifyPass == PasswordVerificationResult.Success && checkUser.RoleId == 1)
                {
                    identity = new ClaimsIdentity(new[] {
                    new Claim(ClaimTypes.Name,checkUser.Username ),
